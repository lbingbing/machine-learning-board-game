...
class OthelloPolicyValueNetModel(PolicyValueNetModel):
    ...
    def create_policyvaluenet_P_network(self):
        dense_t = tf.layers.dense(inputs = self.policyvaluenet_conv_out_t, units = 64, activation = tf.nn.relu)
        self.policyvaluenet_P_out_t = tf.layers.dense(inputs = dense_t, units = 64, activation = tf.nn.relu)

    def create_policyvaluenet_V_network(self):
        dense_t = tf.layers.dense(inputs = self.policyvaluenet_conv_out_t, units = 64, activation = tf.nn.relu)
        self.policyvaluenet_V_out_t = tf.layers.dense(inputs = dense_t, units = 64, activation = tf.nn.relu)
