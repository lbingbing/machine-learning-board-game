...
public:
    ...
    Node(const State& state, int player_id, const typename State::Action& action, Node* parent) : m_player_id(player_id), m_action(action), m_N(0), m_W(0), m_Q(0), m_unexpanded_actions(state.get_legal_actions(State::get_next_player_id(player_id))), m_children{}, m_parent(parent) {}
...
class MctsTree {
public:
    MctsTree(const State& root_state, int player_id, int sim_num) : m_sim_num(sim_num), m_root_node(std::make_unique<Node<State>>(root_state, State::get_next_player_id(player_id), typename State::Action{}, nullptr)), m_root_state(root_state.clone()) {}
...
typename State::Action MctsTree<State>::get_action(const State& state, int player_id) {
    ...
    for (int i = 0; i < m_sim_num; ++i) {
        ...
        std::unique_ptr<State> state{m_root_state->clone()};
        ...
        if (!state->is_end()) {
            ...
            int next_player_id = State::get_next_player_id(node->get_player_id());
...
template <class State>
std::map<typename State::Action, int> search(Node<State> root_node, State& root_state, int sim_num) {
    std::default_random_engine random_generator(std::chrono::system_clock::now().time_since_epoch().count());

    std::unique_ptr<State> root_state1{root_state.clone()};
    for (int i = 0; i < sim_num; ++i) {
        Node<State>* node{&root_node};
        std::unique_ptr<State> state{root_state1->clone()};
